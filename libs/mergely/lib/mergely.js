            ...
            t.extend(s.CodeifyText.prototype, {
                ...
                _codeify: function(t, e) {
                    ...
                    for (var i = 0; i < t.length; ++i) {
                        ...
                        null != r ? e.codes[i] = r : (this._max_code++,
            ...
            t.extend(s.diff.prototype, {
                ...
                normal_form: function() {
                    for (var t = "", e = 0; e < this.items.length; ++e) {
                        ...
                        if (n && r) {
                            var h;
                            for (h = i.lhs_start; h < i.lhs_start + i.lhs_deleted_count; ++h)
                                t += "< " + r[h] + "\n";
                            for (i.rhs_inserted_count && i.lhs_deleted_count && (t += "---\n"),
                            h = i.rhs_start; h < i.rhs_start + i.rhs_inserted_count; ++h)
                                t += "> " + n[h] + "\n"
                        }
                    }
                    return t
                ...
                _lcs: function(t, e, i, s, r, n, h, o) {
                    for (; e < i && r < n && t.codes[e] == s.codes[r]; )
                        ++e,
                        ++r;
                    for (; e < i && r < n && t.codes[i - 1] == s.codes[n - 1]; )
                        --i,
                        --n;
                    if (e == i)
                        for (; r < n; )
                ...
                _sms: function(t, e, i, s, r, n, h, o) {
                    ...
                    for (p = 0; p <= u; ++p) {
                        for (m = a - p; m <= a + p; m += 2) {
                            for (m == a - p ? _ = o[g + m + 1] : (_ = o[g + m - 1] + 1,
                            m < a + p && o[g + m + 1] >= _ && (_ = o[g + m + 1])),
                            v = _ - m; _ < i && v < n && t.codes[_] == s.codes[v]; )
                                _++,
                                v++;
                            if (o[g + m] = _,
                            d && c - p < m && m < c + p && h[f + m] <= o[g + m])
                                return y.x = o[g + m],
                                y.y = o[g + m] - m,
                                y
                        }
                        for (m = c - p; m <= c + p; m += 2) {
                            for (m == c + p ? _ = h[f + m - 1] : (_ = h[f + m + 1] - 1,
                            m > c - p && h[f + m - 1] < _ && (_ = h[f + m - 1])),
                            v = _ - m; _ > e && v > r && t.codes[_ - 1] == s.codes[v - 1]; )
                                _--,
                                v--;
                            if (h[f + m] = _,
                            !d && a - p <= m && m <= a + p && h[f + m] <= o[g + m])
                                return y.x = o[g + m],
                                y.y = o[g + m] - m,
                                y
                        }
                ...
                _optimize: function(t) {
                    for (var e = 0, i = 0; e < t.codes.length; ) {
                        for (; e < t.codes.length && (null == t.modified[e] || 0 == t.modified[e]); )
                            e++;
                        for (i = e; i < t.codes.length && 1 == t.modified[i]; )
                            i++;
                        i < t.codes.length && t.codes[e] == t.codes[i] ? (t.modified[e] = !1,
                        t.modified[i] = !0) : e = i
                    }
                },
                _create_diffs: function(t, e) {
                    for (var i = [], s = 0, r = 0, n = 0, h = 0; n < t.codes.length || h < e.codes.length; )
                        if (n < t.codes.length && !t.modified[n] && h < e.codes.length && !e.modified[h])
                            n++,
                            h++;
                        else {
                            for (s = n,
                            r = h; n < t.codes.length && (h >= e.codes.length || t.modified[n]); )
                                n++;
                            for (; h < e.codes.length && (n >= t.codes.length || e.modified[h]); )
                                h++;
                            (s < n || r < h) && i.push({
                                lhs_start: s,
                                rhs_start: r,
            ...
            t.extend(s.CodeMirrorDiffView.prototype, {
                init: function(e, i) {
                    this.settings = t.extend(!0, {
                        ...
                        loaded: function() {},
                        resize: function(i) {
                            var s, r, n = t(e).parent(), h = (s = "auto" == this.width ? n.width() : this.width) / 2 - 16 - 8, o = r = "auto" == this.height ? n.height() - 2 : this.height, l = t(e);
                ...
                scrollToDiff: function(t) {
                    this.changes.length && ("next" == t ? this._current_diff == this.changes.length - 1 ? this._current_diff = 0 : this._current_diff = Math.min(++this._current_diff, this.changes.length - 1) : "prev" == t && (0 == this._current_diff ? this._current_diff = this.changes.length - 1 : this._current_diff = Math.max(--this._current_diff, 0)),
                ...
                scrollTo: function(t, e) {
                    ...
                    "lhs" == t ? (i.setCursor(e),
                    i.centerOnCursor()) : (s.setCursor(e),
                    s.centerOnCursor())
                },
                _setOptions: function(e) {
                    if (t.extend(this.settings, e),
                    ...
                    this.settings.hasOwnProperty("wrap_lines") && this.editor && (s = this.editor[this.id + "-lhs"],
                    r = this.editor[this.id + "-rhs"],
                    s.setOption("lineWrapping", this.settings.wrap_lines),
                    r.setOption("lineWrapping", this.settings.wrap_lines)),
                    this.settings.hasOwnProperty("line_numbers") && this.editor && (s = this.editor[this.id + "-lhs"],
                    r = this.editor[this.id + "-rhs"],
                    s.setOption("lineNumbers", this.settings.line_numbers),
                    r.setOption("lineNumbers", this.settings.line_numbers))
                },
                options: function(t) {
                    if (!t)
                ...
                search: function(t, e, i) {
                    var s, r = this.editor[this.id + "-lhs"], n = this.editor[this.id + "-rhs"];
                ...
                bind: function(i) {
                    ...
                    null != t.button)
                        s = '<button title="Merge left"></button>',
                        r = '<button title="Merge right"></button>';
                    else {
                        var n = "opacity:0.6;height:16px;background-color:#bfbfbf;cursor:pointer;text-align:center;color:#eee;border:1px solid #848484;margin-right:-15px;margin-top:-2px;";
                        s = '<div style="' + n + '" title="Merge left">&lt;</div>',
                        r = '<div style="' + n + '" title="Merge right">&gt;</div>'
                    }
                    this.merge_rhs_button = t(r),
                    this.merge_lhs_button = t(s);
                    var h = t('<div id="mergely-splash">')
                      , o = t('<div class="mergely-margin" style="height: \''.concat("10px", '\'"><canvas id="lhs-margin" width="8px" height="\'').concat("10px", "'\"></canvas></div>"));
                    o.find("#lhs-margin").attr("id", "".concat(this.id, "-lhs-margin"));
                    var l = t("<div style=\"position:relative;width:'".concat("10px", "'; height:'").concat("10px", '\'" id="editor-lhs" class="mergely-column"><textarea id="text-lhs"></textarea></div>'));
                    l.eq(0).attr("id", "".concat(this.id, "-editor-lhs")),
                    l.find("#text-lhs").attr("id", "".concat(this.id, "-lhs"));
                    var a = t('<div class="mergely-canvas" style="height: \''.concat("10px", '\'"><canvas id="lhs-rhs-canvas" style="width:28px" width="28px" height="\'').concat("10px", "'\"></canvas></div>"));
                    a.find("#mergely-canvas").attr("id", "".concat(this.id, "-mergely-canvas")),
                    a.find("#lhs-rhs-canvas").attr("id", "".concat(this.id, "-lhs-").concat(this.id, "-rhs-canvas")),
                    this.element.append(h),
                    this.element.append(o),
                    this.element.append(l),
                    this.element.append(a);
                    var c = t('<div class="mergely-margin" style="height: \''.concat("10px", '\'"><canvas id="rhs-margin" width="8px" height="\'').concat("10px", "'\"></canvas></div>"));
                    c.find("#rhs-margin").attr("id", "".concat(this.id, "-rhs-margin")),
                    "left" == this.settings.rhs_margin && this.element.append(c);
                    var d, g = t("<div style=\"width:'".concat("10px", "'; height:'").concat("10px", '\'" id="editor-rhs" class="mergely-column"><textarea id="text-rhs"></textarea></div>'));
                    if (g.eq(0).attr("id", "".concat(this.id, "-editor-rhs")),
                    g.find("#text-rhs").attr("id", "".concat(this.id, "-rhs")),
                    ...
                    ["lgpl-separate-notice", "gpl-separate-notice", "mpl-separate-notice", "commercial"].indexOf(this.settings.license) < 0) {
                        var f = {
                          ...
                          , u = f[this.settings.license];
                        u || (u = f.lgpl);
                        this.element.parent().height();
                        var p = this.element.parent().width();
                    ...
                    try {
                        d = this.element.find("#".concat(this.id, "-rhs")).get(0)
                    } catch (t) {}
                    if (d) {
                        var m;
                        try {
                            m = this.element.find("#".concat(this.id, "-lhs")).get(0)
                        } catch (t) {}
                        if (m) {
                            var _ = t('<div style="display:none" class="mergely current start" />').appendTo("body").css("border-top-color");
                            this.current_diff_color = _;
                            ...
                            this.settings.autoresize && (v += "".concat(this.id, " .CodeMirror-scroll { height: 100%; overflow: auto; }")),
                            t('<style type="text/css">'.concat(v += "\n.CodeMirror { line-height: 18px; }", "</style>")).appendTo("head");
                            var y, w = this;
                            if (w.trace("init", "binding event listeners"),
                            this.editor = [],
                            this.editor[this.id + "-lhs"] = e.fromTextArea(m, this.lhs_cmsettings),
                            this.editor[this.id + "-rhs"] = e.fromTextArea(d, this.rhs_cmsettings),
                            ...
                            .bind(this)),
                            this.editor[this.id + "-rhs"].on("gutterClick", function(t, e, i, s) {
                                C.call(this, "rhs", e, s)
                            }
                            .bind(this)),
                            this.settings.lhs && (w.trace("init", "setting lhs value"),
                            y = this.editor[this.id + "-lhs"].getDoc().setValue,
                            this.settings.lhs(y.bind(this.editor[this.id + "-lhs"].getDoc()))),
                            this.settings.rhs && (w.trace("init", "setting rhs value"),
                            y = this.editor[this.id + "-rhs"].getDoc().setValue,
                            this.settings.rhs(y.bind(this.editor[this.id + "-rhs"].getDoc()))),
                        ...
                        for (var a in this.editor)
                            if (this.editor.hasOwnProperty(a) && e != a) {
                                for (var c = e.replace(this.id + "-", ""), d = a.replace(this.id + "-", ""), g = 0, f = null, u = !1, p = 0; p < this.changes.length; ++p) {
                                    ...
                                    s.line >= m[c + "-line-from"] && (f = m,
                                    s.line >= f[c + "-line-to"] && (m.hasOwnProperty(c + "-y-start") && m.hasOwnProperty(c + "-y-end") && m.hasOwnProperty(d + "-y-start") && m.hasOwnProperty(d + "-y-end") ? g += m[c + "-y-end"] - m[c + "-y-start"] - (m[d + "-y-end"] - m[d + "-y-start"]) : u = !0))
                                }
                                var _ = this.editor[a].getViewport()
                                  , v = !0;
                                f && (this.trace("scroll", "last change before midline", f),
                                s.line >= _.from && s <= _.to && (v = !1)),
                                this.trace("scroll", "scroll", v),
                                v || u ? (this.trace("scroll", "scrolling other side", n - g),
                                this._skipscroll[a] = !0,
                                this.editor[a].scrollTo(h, n - g)) : this.trace("scroll", "not scrolling other side"),
                                this.settings.autoupdate && (r.start(),
                                this._calculate_offsets(o, l, this.changes),
                                this.trace("change", "offsets time", r.stop()),
                                this._markup_changes(o, l, this.changes),
                                this.trace("change", "markup time", r.stop()),
                                this._draw_diff(o, l, this.changes),
                                this.trace("change", "draw time", r.stop())),
                                this.trace("scroll", "scrolled")
                            }
                    } else
                        this._skipscroll[e] = !1
                },
                ...
                _clear: function() {
                    var t, e, i, s, n, h, o = this, l = function() {
                        for (r.start(),
                        s = 0,
                        h = e.lineCount(); s < h; ++s)
                            e.removeLineClass(s, "background");
                        for (s = 0; s < i.length; ++s)
                            (n = i[s]).lines.length && o.trace("change", "clear text", n.lines[0].text),
                            n.clear();
                        e.clearGutter("merge"),
                        o.trace("change", "clear time", r.stop())
                    };
                    for (t in this.editor)
                        this.editor.hasOwnProperty(t) && (e = this.editor[t],
                        i = o.chfns[t],
                        e.operation(l));
                    o.chfns[t] = [];
                ...
                _diff: function(t, e) {
                    ...
                    void 0 === this._current_diff && this.changes.length && (this._current_diff = 0,
                ...
                _set_top_offset: function(t) {
                    ...
                    return !!i && (this.editor[t].scrollTo(null, e),
                    this.draw_top_offset = .5 - i,
                    !0)
                },
                _calculate_offsets: function(e, i, s) {
                    if (null == this.em_height) {
                        if (!this._set_top_offset(e))
                            return;
                        this.em_height = this.editor[e].defaultTextHeight(),
                        this.em_height || (console.warn("Failed to calculate offsets, using 18 by default"),
                        this.em_height = 18),
                        this.draw_lhs_min = .5;
                        var r = t("#" + e + "-" + i + "-canvas");
                        if (r.length || console.error("failed to find canvas", "#" + e + "-" + i + "-canvas"),
                        !r.width())
                    ...
                    for (var n = this.editor[e].charCoords({
                        ...
                        line: 0
                    }), o = this._get_viewport_side(e), l = (this._get_viewport_side(i),
                    0); l < s.length; ++l) {
                        var a = s[l];
                        if (!this.settings.viewport || this._is_change_in_view(o, "lhs", a) || this._is_change_in_view(o, "rhs", a)) {
                            var c, d, g, f, u, p, m, _, v, y, w = a["lhs-line-from"] >= 0 ? a["lhs-line-from"] : 0, b = a["lhs-line-to"] >= 0 ? a["lhs-line-to"] : 0, x = a["rhs-line-from"] >= 0 ? a["rhs-line-from"] : 0, C = a["rhs-line-to"] >= 0 ? a["rhs-line-to"] : 0;
                            this.editor[e].getOption("lineWrapping") || this.editor[i].getOption("lineWrapping") ? (u = this.editor[e].cursorCoords({
                                line: w,
                                ch: 0
                            }, "page"),
                            _ = this.editor[e].getLineHandle(w),
                            c = {
                                top: u.top,
                            ...
                            m = this.editor[e].getLineHandle(b),
                            d = {
                                top: p.top,
                            ...
                            v = this.editor[i].getLineHandle(x),
                            g = {
                                top: u.top,
                            ...
                            y = this.editor[i].getLineHandle(C),
                            f = {
                                top: p.top,
                            ...
                            "a" == a.op ? x > 0 && (c.top = c.bottom,
                            c.bottom += this.em_height,
                            d = c) : "d" == a.op && w > 0 && (g.top = g.bottom,
                            g.bottom += this.em_height,
                            f = g),
                ...
                _markup_changes: function(e, i, n) {
                        ...
                        for (var t = 0; t < n.length; ++t) {
                            ...
                            if (this._is_change_in_view("lhs", c, e)) {
                                var i = e["lhs-line-from"] >= 0 ? e["lhs-line-from"] : 0
                                  ...
                                  , d = (e["rhs-line-to"] >= 0 && e["rhs-line-to"],
                                ["mergely", "lhs", e.op, "cid-" + t]);
                                if (o.addLineClass(i, "background", "start"),
                                o.addLineClass(s, "background", "end"),
                                e["lhs-line-from"] < 0 && d.push("empty"),
                                a == t && (i != s && o.addLineClass(i, "background", "current"),
                                o.addLineClass(s, "background", "current")),
                                0 == i && 0 == s && 0 == r)
                                    o.addLineClass(i, "background", d.join(" ")),
                    ...
                    l.operation(function() {
                        for (var t = 0; t < n.length; ++t) {
                            ...
                            if (this._is_change_in_view("rhs", d, e)) {
                                var i = e["lhs-line-from"] >= 0 ? e["lhs-line-from"] : 0
                                  , s = (e["lhs-line-to"] >= 0 && e["lhs-line-to"],
                                e["rhs-line-from"] >= 0 ? e["rhs-line-from"] : 0)
                                  , r = e["rhs-line-to"] >= 0 ? e["rhs-line-to"] : 0
                                  , c = ["mergely", "rhs", e.op, "cid-" + t];
                                if (l.addLineClass(s, "background", "start"),
                                l.addLineClass(r, "background", "end"),
                                e["rhs-line-from"] < 0 && c.push("empty"),
                                a == t && (s != r && l.addLineClass(s, "background", "current"),
                                l.addLineClass(r, "background", "current")),
                                0 == s && 0 == r && 0 == i)
                                    l.addLineClass(s, "background", c.join(" ")),
                        ...
                        if ("d" == u.op) {
                            ...
                            ++g) {
                                ...
                                if (g > v)
                                    ...
                                    x = o.getLine(k),
                                    C = l.getLine(g),
                                    new s.LCS(x,C).diff(function(t, e) {
                    ...
                    this.element.find(".merge-button").on("click", function(e) {
                        var i = "rhs"
                          , s = "lhs";
                        t(this).parents("#" + h.id + "-editor-lhs").length && (i = "lhs",
                        s = "rhs");
                        ...
                        t.each(o.bgClass.split(" "), function(t, e) {
                            if (0 == e.indexOf("cid-"))
                                return n = parseInt(e.split("-")[1], 10),
                                !1
                        });
                        var l = h.changes[n];
                    ...
                    var M, O, L, k, P = t("#mergely-lhs ~ .CodeMirror .CodeMirror-code .CodeMirror-linenumber.CodeMirror-gutter-elt"), T = t("#mergely-rhs ~ .CodeMirror .CodeMirror-code .CodeMirror-linenumber.CodeMirror-gutter-elt");
                    ...
                    for (L = 0; L < n.length; ++L) {
                        if (u = n[L],
                        a == L && "a" !== u.op)
                            for (M = u["lhs-line-from"],
                            O = u["lhs-line-to"] + 1,
                            k = M; k < O; ++k)
                                k >= S && k <= E && P.eq(k - S).addClass("mergely current");
                        if (a == L && "d" !== u.op)
                            for (M = u["rhs-line-from"],
                            O = u["rhs-line-to"] + 1,
                            k = M; k < O; ++k)
                                k >= V && k <= j && T.eq(k - V).addClass("mergely current")
                    }
                ...
                _merge_change: function(t, i, s) {
                    if (t) {
                        ...
                        }, h = t[i + "-line-from"], o = t[i + "-line-to"], l = t[s + "-line-from"], a = t[s + "-line-to"], c = n[i].getDoc(), d = n[s].getDoc(), g = h >= 0 ? c.getLine(h).length + 1 : 0, f = o >= 0 ? c.getLine(o).length + 1 : 0, u = a >= 0 ? d.getLine(a).length + 1 : 0, p = l >= 0 ? d.getLine(l).length + 1 : 0;
                        "c" === t.op ? (r = c.getRange(e.Pos(h, 0), e.Pos(o, f)),
                        d.replaceRange(r, e.Pos(l, 0), e.Pos(a, u))) : "lhs" === s && "d" === t.op || "rhs" === s && "a" === t.op ? (h > 0 ? (r = c.getRange(e.Pos(h, g), e.Pos(o, f)),
                        l += 1) : r = c.getRange(e.Pos(0, 0), e.Pos(o + 1, 0)),
                        d.replaceRange(r, e.Pos(l - 1, 0), e.Pos(a + 1, 0))) : ("rhs" === s && "d" === t.op || "lhs" === s && "a" === t.op) && (h > 0 ? (g = c.getLine(h - 1).length + 1,
                        r = c.getRange(e.Pos(h - 1, g), e.Pos(o, f))) : r = c.getRange(e.Pos(0, 0), e.Pos(o + 1, 0)),
                        l < 0 && (l = 0),
                        d.replaceRange(r, e.Pos(l, p))),
                ...
                _draw_info: function(e, i) {
                    ...
                    if (null == n)
                        throw "Failed to find: " + e + "-" + i + "-canvas";
                ...
                _draw_diff: function(e, i, s) {
                    ...
                    for (var c = this._get_viewport_side(e), d = this._get_viewport_side(i), g = 0; g < s.length; ++g) {
                        ...
                        if (this.trace("draw", "marker calculated", p, m, _, v),
                        l.beginPath(),
                        l.fillStyle = u,
                        l.strokeStyle = "#000",
                        l.lineWidth = .5,
                        l.fillRect(1.5, p, 4.5, Math.max(m - p, 5)),
                        l.strokeRect(1.5, p, 4.5, Math.max(m - p, 5)),
                        a.beginPath(),
                        a.fillStyle = u,
                        a.strokeStyle = "#000",
                        a.lineWidth = .5,
                        a.fillRect(1.5, _, 4.5, Math.max(v - _, 5)),
                        a.strokeRect(1.5, _, 4.5, Math.max(v - _, 5)),
                        this._is_change_in_view("lhs", c, f) || this._is_change_in_view("rhs", d, f)) {
                            p = f["lhs-y-start"],
                            m = f["lhs-y-end"],
                            _ = f["rhs-y-start"],
                            v = f["rhs-y-end"];
                            ...
                            var y = this.draw_lhs_width
                              ...
                              , x = p;
                            o.moveTo(b, x),
                            "Microsoft Internet Explorer" == navigator.appName ? (o.lineTo(this.draw_lhs_min + this.draw_lhs_width, p),
                            o.lineTo(this.draw_lhs_min + this.draw_lhs_width, m + 1),
                            o.lineTo(this.draw_lhs_min, m + 1)) : (w <= 0 ? o.lineTo(b + y, x) : (o.arcTo(b + y, x, b + y, x + 3, 3),
                            o.arcTo(b + y, x + w, b + y - 3, x + w, 3)),
                            o.lineTo(b, x + w)),
                            o.stroke(),
                            ...
                            x = _,
                            o.moveTo(b, x),
                            "Microsoft Internet Explorer" == navigator.appName ? (o.lineTo(this.draw_rhs_max - this.draw_rhs_width, _),
                            o.lineTo(this.draw_rhs_max - this.draw_rhs_width, v + 1),
                            o.lineTo(this.draw_rhs_max, v + 1)) : (w <= 0 ? o.lineTo(b - y, x) : (o.arcTo(b - y, x, b - y, x + 3, 3),
                            o.arcTo(b - y, x + w, b - 3, x + w, 3)),
                            o.lineTo(b, x + w)),
                            o.stroke();
                ...
                trace: function(t) {
                    this.settings._debug.indexOf(t) >= 0 && (arguments[0] = t + ":",
                    console.log([].slice.apply(arguments)))
                }
            }),
            t.pluginMaker = function(e) {
                t.fn[e.prototype.name] = function(i) {
                    var s, r = t.makeArray(arguments), n = r.slice(1);
