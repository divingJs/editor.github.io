    ...
    var widget = {
        ...
        ajustaMargen: function(p, data, elem) {
            ...
            diving.each(p.dRoles, function(i, v) {
                ...
                switch (v) {
                    case 't':
                        stl += "n-resize; top:" + vth.y + "px;left:" + vth.left + "px;width:" + vth.width + "px;height:6px;";
                        break;
                    case 'tr':
                        stl += "ne-resize;top:" + vth.y + "px;left:" + (vth.x + vth.width) + "px;width:6px;height:6px;";
                        break;
                    case 'r':
                        stl += "ew-resize;top:" + vth.y + "px;left:" + (vth.x + vth.width) + "px;width:6px;height:" + vth.height + "px;";
                        break;
                    case 'tl':
                        stl += "nw-resize;top:" + vth.y + "px;left:" + vth.x + "px;width:6px;height:6px;";
                        break;
                    case 'l':
                        stl += "e-resize; top:" + vth.y + "px;left:" + vth.x + "px;width:6px;height:" + vth.height + "px;";
                        break;
                    case 'b':
                        stl += "n-resize; top:" + (vth.y + vth.height) + "px;left:" + vth.left + "px;width:" + vth.width + "px;height:6px;";
                        break;
                    case 'br':
                        stl += "nw-resize;top:" + (vth.y + vth.height) + "px;left:" + (vth.x + vth.width) + "px;width:6px;height:6px;";
                        break;
                    case 'bl':
                        stl += "ne-resize;top:" + (vth.y + vth.height) + "px;left:" + vth.x + "px;width:6px;height:6px;";
                        break;
                }
...
/*(function(){
     var widget = {
        ...
        createElement: function(p,data) {
            ...
            var resizeWindow = dvn('<div>',{
                    'd-role':'resizeWindow',
                    text:'&nbsp;',
                    click:function(){
                    }
                });
            contentVentana.append(resizeWindow);
            var px, py;
            var obj = resizeWindow[0];
            var dragObj=null;
            resizeWindow[0].addEventListener('mousedown',function(a){
                px = a.layerX;
                py = a.layerY;
                dragObj = obj;
            })
            ...
            obj.addEventListener('mousemove', function (e) {
                var vw = e.pageX-((parseFloat(contentVentana[0].style.left.replace('px',''))+contentVentana[0].clientWidth)-(this.clientWidth-px));
                var tv = (contentVentana[0].style.top!='')?parseFloat(contentVentana[0].style.top.replace('px','')):0;
                var hv = contentVentana[0].clientHeight;
                var ht = this.clientHeight;
                var vh = (e.pageY-((tv+hv-ht)+py));
                if(dragObj==null)return;
                if(p.resize){data.resize((contentVentana[0].clientWidth+vw),(contentVentana[0].clientHeight+vh));}
                var stl = dvn(contentVentana[0]).attr('style');
                stl = /width:/.test(stl)?stl.replace(/width:\s?[0-9\.]+px;/,'width:'+(contentVentana[0].clientWidth+vw)+'px;'):stl+'width:'+(contentVentana[0].clientWidth+vw)+'px;';
                stl = /height:/.test(stl)?stl.replace(/height:\s?[0-9\.]+px;/,'height:'+(contentVentana[0].clientHeight+vh)+'px;'):stl+'height:'+(contentVentana[0].clientHeight+vh)+'px;';
                dvn(contentVentana[0]).attr('style',stl);
            });
    ...
    var widget = {
        ...
        addElement: function(obj, component) {
            ...
            var li = diving('<li>', {
                ...
                click: function() {
                    dlv.selected = [{
                        value: diving(this).attr('d-value'),
                        text: diving(this).text()
                    }];
                    if (dlv.click) {
