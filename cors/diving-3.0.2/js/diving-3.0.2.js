(function () {
    ...
    diving.extend({
        ...
        camelCase:function(string){
            var rt = string.replace(rdashAlpha,fcamelCase);
            return rt;
        },
        exportTo:{
            CSV:function(obj){
                var hd =''+((typeof obj.head[0]==='string')?obj.head.join(','):(dvn.map(obj.head,function(i){return i[(obj.hasOwnProperty('label')?obj.label:obj.key)];})));
                var bd=''+dvn.map(obj.data,function(i){return (typeof i==='string')?('\n'+i):('\n'+(dvn.map(Object.keys(i),function(j){return i[j];})));});
                var csvText=hd+bd;
            ...
            XLS:function(obj){
                ...
                if(obj.hasOwnProperty('head')){
                    ...
                    var thr=dvn('<tr>');
                    dvn('body').append(t);
                    t.append(th.append(thr));
                    for(var i=0;i<obj.head.length;i++){
                        thr.append(dvn('<th>',{text:obj.head[i][(obj.hasOwnProperty('label')?obj.label:obj.key)]}));
                    }
                    for(var i=0;i<obj.data.length;i++){
                        var k = Object.keys(obj.data[i]);
                        var tr=dvn('<tr>');
                        for(var j=0;j<k.length;j++){
                            tr.append(dvn('<td>',{text:obj.data[i][k[j]]}));
                        }
                        tb.append(tr);
                    }
                    t.append(tb);
                ...
                var ctx = {worksheet:obj.name||'Worksheet',table: t[0].innerHTML};
                var link = document.createElement("a");
                    link.download = obj.name+(/(.xls)$/.test(obj.name)?'':".xls");
                    link.href = uri + base64(format(template, ctx));
                    link.click();
                if(obj.hasOwnProperty('head')){t.remove();}
            }
        },
