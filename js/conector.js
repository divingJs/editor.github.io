...
function dashboard(retorno,consulta){socket.emit('consulta','saludo',retorno,consulta);}
function removeDashboard(){dvn('#dashboard').remove();}
async function clearDashboard(){
	if(dvn('#dashboard').length>0){dvn('#dashboard').remove();}
	dvn('body').append(dvn('<div>',{id:'dashboard'}));
	return true;
}
